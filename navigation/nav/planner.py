...
def global_plan(
    ...
    open_set.put(node_now)
    closed_set = set()
    g_score = np.full(costmap.shape, float("inf"))
    g_score[node_now.position[1], node_now.position[0]] = 0.
    while not open_set.empty():
        node_now = open_set.get()
        if node_now.position in closed_set:
            continue
        if node_now.g > g_score[node_now.position[1], node_now.position[0]]:
            continue
